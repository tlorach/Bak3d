...
static std::string getLine(char * buf, int size, FILE * fp)
{
	do {
		buf[size-1] = '$';
		fgets(buf, size, fp);
...
bool Model::loadObjFromFile( const char *file, Model &m) {
    ...
    while ( fscanf( fp, "%s", buf) != EOF ) {
        ...
        {
            m._materials.push_back(OffsetAndName(m._pIndex.size(), std::string(getLine(buf, BUF_SIZE, fp)) ));
        }
        else if(!strncmp(buf, "mtllib", 6))
        {
            std::string name = getLine(buf, BUF_SIZE, fp);
            printf(name.c_str());
            // open mtl file
        }
        else switch (buf[0]) {
            ...
            case 's':
                ...
                m._smoothGroups.push_back(Smooth(m._pIndex.size(), buf[1]=='1' ? true:false ));
                break;
            case 'g':
