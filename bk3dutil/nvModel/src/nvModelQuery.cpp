...
const float* Model::getCompiledAttributes(int &stride
                ,int &nOffset
                ,int &tcOffset
                ,int &sTanOffset
                ,int &cOffset
                ) const
{
    ...
    nOffset = _nOffset;
    tcOffset = _tcOffset;
    sTanOffset = _sTanOffset;
    cOffset = _cOffset;
    stride = _vtxSize;
    return (_vertices.size() > 0) ? &(_vertices[0]) : NULL;
}
//
//
////////////////////////////////////////////////////////////
...
int Model::getSmoothGroupCount() const
{
    return (int)_smoothGroups.size();
}
bool Model::getObject(int n, int *offset, const char** name) const
{
    if(n>=_objects.size())
        return false;
    if(offset) offset[0] = _objects[n].offset;
    if(name) name[0] = _objects[n].name.c_str();
    return true;
}
bool Model::getGroup(int n, int *offset, const char** name) const
{
    if(n>=_groups.size())
        return false;
    if(offset) offset[0] = _groups[n].offset;
    if(name) name[0] = _groups[n].name.c_str();
    return true;
}
bool Model::getMaterial(int n, int *offset, const char** name) const
{
    if(n>=_materials.size())
        return false;
    if(offset) offset[0] = _materials[n].offset;
    if(name) name[0] = _materials[n].name.c_str();
    return true;
}
bool Model::getSmoothGroup(int n, int *offset, bool* mode) const
{
    if(n>=_smoothGroups.size())
        return false;
    if(offset) offset[0] = _smoothGroups[n].offset;
    if(mode) mode[0] = _smoothGroups[n].mode;
    return true;
}
