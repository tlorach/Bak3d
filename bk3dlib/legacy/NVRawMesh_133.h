 ...
 **/
#define RAWMESHVERSION133 0x133
 ...
 ** @{
 **/
#define NODE133_HEADER     0 ///< \brief the very first node that you should find in the binary file
#define NODE133_MESH       1 ///< \brief the mesh node
#define NODE133_TRANSFORM  2
#define NODE133_JOINT      3
#define NODE133_TRANSFPOOL 4
#define NODE133_ATTRIBUTE  5
#define NODE133_SLOT		6
#define NODE133_PRIMGROUP  7
#define NODE133_RELOCTABLE 8
#define NODE133_MAYACURVE  9
#define NODE133_MAYACURVEVECTOR  10
#define NODE133_FLOATARRAY 11
#define NODE133_MATERIAL   12
#define NODE133_IKHANDLE   13
#define NODE133_DOF        14
//...
#define NODE133_END		15
/// @}
//
...
struct Node
{
	short               nodeType; 		///< See \ref NODE133_HEADER
	short				version;		///< \brief Node version. Most of the time we expect all to be the same version. NODE133_HEADER's version is the most relevant.
...
///
/// \brief connector NODE133_FLOATARRAY for vectors/matrices/scalars of Floats.
///
...
struct FloatArray : public Node
{
	...
	FloatArray() {		
		...
		nodeType = NODE133_FLOATARRAY;
		version = RAWMESHVERSION133;
...
struct MayaCurveVector : public Node
{
	...
	MayaCurveVector()
	{
		...
		nodeType = NODE133_MAYACURVEVECTOR;
		version = RAWMESHVERSION133;
	}
};
...
----------------------------------*/
  ...
  ///
  /// \brief Transform (NODE133_TRANSFORM) : a Maya-centric transformation system
  ///
...
// TODO : connection with animation curves, if existing
	  ...
	  void debugDumpLayout(int l, int level, const char * nodeNameFilter);
  };
...
///
/// \brief Attribute of a vertex (NODE133_ATTRIBUTE) : data required to setup vertex shader's input attributes
///
...
///
  ...
  /// 
  /// \brief slot/stream (NODE133_SLOT) : made of interleaved attributes. In the easiest case it has only one attribute.
  ///
...
///
/// \brief Material (NODE133_MATERIAL) containing usual properties
///
  ...
  **/
  struct Mesh : public Node
  {
	...
	void init()
    {
      ...
      nodeType = NODE133_MESH;
	  version = RAWMESHVERSION133;
    }
    void resolvePointers();
    void debugDumpLayout(unsigned int &nVtx, unsigned int &nElts, unsigned int &nPrims, int level, const char * nodeNameFilter);
  };
  ...
  struct RelocationTable : public Node
  {
	...
	void init()
    {
      ...
      nodeType = NODE133_RELOCTABLE;
	  version = RAWMESHVERSION133;
    }
  };
  //------------------------------------------------------------------------------------------
  /// \brief Header (NODE133_HEADER) of the Bk3d file. This where the main data are accessible
  ///
  ...
  struct FileHeader : public Node
  {
    ...
    void          debugDumpAll(int level=1, const char * nodeNameFilter = NULL);
  };
} //namespace bk3d133
...
// 
//------------------------------------------------------------------------------------------
INLINE void FileHeader::debugDumpAll(int level, const char * nodeNameFilter)
{
	if(pTransforms) 
		for(int i=0; i< pTransforms->n; i++)
		{
			pTransforms->p[i]->debugDumpLayout(0, level, nodeNameFilter);
		}
	PRINTF((TEXT("\n")));
	if(pMaterials)
	...
	if(pMeshes)
		...
		{
			pMeshes->p[i]->debugDumpLayout(nVtx, nElts, nPrims, level, nodeNameFilter);
		}
...
// 
//------------------------------------------------------------------------------------------
INLINE void Mesh::debugDumpLayout(unsigned int &nVtx, unsigned int &nElts, unsigned int &nPrims, int level, const char * nodeNameFilter)
{
  if(nodeNameFilter && (strcmp(nodeNameFilter, name)))
      return;
  if(level==0)
...
#   define RAWMESHMINSZ (1024*1000)
    ...
    //  EPRINTF((TEXT("Error : Not a mesh file\n")));
	   ...
	   // return false;
    //}
    if(((FileHeader *)memory)->version != RAWMESHVERSION133)
    {
      PRINTF((TEXT("Error>> Wrong version in Mesh description\n")));
      PRINTF((TEXT("needed %x and got %x\n"), RAWMESHVERSION133, ((FileHeader *)memory)->version));
      free(memory);
      return NULL;
    }
