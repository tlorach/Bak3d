        ...
        } else
        {
            ...
            }
            {
                ...
                else if(tr->validComps & TRANSFCOMP_rotation)
                {
                    ...
                    vec3 e;
if(!strcmp(tr->name, "l_elbow"))
   e = vec3(90,0,0);
else
    e = vec3(0,0,0);
quat qz(vec3(0,0,1), e.z * nv_to_rad);
quat qy(vec3(0,1,0), e.y * nv_to_rad);
quat qx(vec3(1,0,0), e.x * nv_to_rad);
...
void updateTransforms(bk3d::FileHeader * bk3dHeader, bool bComputeBindPose=false, bool forceDirty=false)
{
    if(bk3dHeader && bk3dHeader->pTransforms)
        ...
        {
          if(bk3dHeader->pTransforms->p[i]->pParent == NULL)
