...
//
// A special way to compile models:
// for each vertex-pos, create a unique tc and normal etc.
// This is used for when exporting an OBJ file for quick editing in
// Blender/ZBrush without changing the vertex order: Blender/ZBrush
// seem to optimize tc and normal while keeping original vertices
// THIS ASSUMES:
// - only one combination is available for one vertex position
// - triangles must use all the vertices
//////////////////////////////////////////////////////////////////////
void Model::compileModel2() {
    ...
    _vertices.resize(_positions.size()*_vtxSize);

    // Walk through triangles and adjust vertices
    ...
    while ( pit < _pIndex.end()) {
        ...
        _indices[2].push_back(*pit);

        idxv = *pit;
        float* attrs = &_vertices[(*pit)*_vtxSize];
        float *pos = &_positions[(*pit)*_posSize];
        memcpy(attrs, pos, sizeof(float)*_posSize);
        attrs += _posSize;
        if (hasNormals()) {
            idxn = *nit;
            float* n = &_normals[(*nit)*3];
            memcpy(attrs, n, sizeof(float)*3);
            attrs += 3;
        }
        if (hasTexCoords()) {
            idxtc = *tit;
            float* tc = &_texCoords[(*tit)*_tcSize];
            memcpy(attrs, tc, sizeof(float)*_tcSize);
            attrs += _tcSize;
        }
        if (hasColors()) {
            idxc = *cit;
            float* c = &_colors[(*cit)*_cSize];
            memcpy(attrs, c, sizeof(float)*_cSize);
            attrs += _cSize;
        }
        if (hasTangents()) {
            idxtan = *tanit;
            float* t = &_sTangents[(*tanit)*3];
            memcpy(attrs, t, sizeof(float)*3);
            attrs += 3;
        }
...
//
//////////////////////////////////////////////////////////////////////
void Model::computeBoundingBox( float *minVal_, float *maxVal_) {

    vec3f minVal(minVal_);
    vec3f maxVal(maxVal_);
    if ( _positions.empty())
        return;
...
void Model::rescale( float radius) {
    ...
    vec3f minVal, maxVal;
    computeBoundingBox(minVal._array, maxVal._array);
